...
### 2.3.2 Introdução
A estatística Bayesiana é baseada no Teorema de Bayes, que é descrito como P(A|B) = P(A).P(B|A)/P(B).O termo P(A|B), ou probabilidade de A dado B, é a chamada probabilidade posterior, P(A) é a probabilidade a priori, P(B|A) é a verossimilhança e P(B) é a probabilidade marginal, ela tem função de normalizar as quantidades e é calculada através de uma integral.
...
E qual a nossa verossimilhança? Qual a probabilidade de testar positivo (nossos dados), dado que o indivíduo está doente (nossa hipótese)? Nossa verossimilhança P(P|I) é a nossa sensibilidade! Portanto:
```{r}
prob_PI <- 0.93
```
...
Para isso temos que pensar da seguinte maneira, as propabilidades de se estar grávida tomando o anticoncepcional devem variar de 0 a 1, isto é, de 0% a 100%. A verossimilhança deve ser baseada nós nossos dados, ou seja, a cada 20 mulheres que tomaram o anticoncepcional em média 4 ficam grávidas, isso significa dizer que a verossimilhança é uma distribuição binominal com tamanho 20 - número de participantes -, média 4 - número de grávidas - e uma probabilidade de sucesso baseada na variação da nossa probabilidade, ou seja entre 0 e 1.

Como é uma situação hipotética podemos estabeler nosso priori, comecemos com um prior não significativo que tenha a mesma probabilidade para todas as possibilidades, ou seja, que seja 1. Por fim probabilidade marginal, como na situação anterior, é igual ao priori * verossimilhança. 
...
Veja que o teorema de Bayes realiza, matematicamente, justamente o que pensamos por inferência, isso é o posterior segue a tendência da verossimilhança, uma vez que o prior não tem significância. A curva do posterior está mais abaixo que a da verossimilhança pois esse é o que chamamos de posterior padronizado ou normalizado, uma vez que ele está sendo divido pela probabilidade marginal. Se tivessemos uma linha nesse gráfico para numerador da equação ela estaria exatamente sobre a linha da verossimilhança. Por fim a linha do posterior aparece constante e em cima justamente pois ele é um para todas as possibilidades.
...
Veja como novamente o teorema ajustou a distribuição do posterior, justamnete como nós intuimos.

Por fim, vamos ao último exemplo dessa situação problema, o que aconteceria se tivessemos dados poucos informativos, ou seja, se tivessemos poucas observações, por exemplo, três com duas grávidas e um priori mais condizente com observações anteriores, ou seja, que descrevesse uma probabilidade de 25% de se estar grávida?
...
Mais uma vez, o teorema adequa a distribuição posterior conforme a intuição, ou seja o mais próximo de nosso conhecimento prévio que nesse caso é muito mais confiavel que nossos dados. Essa, inclusive é uma das vantagens de se usar a inferência bayesiana, que com observações muito pequenas, a utilização de um priori adequa a probabilidade posterior. Vale ressaltar que caso priori e verossimilhança sejam condizentes o posterior vai se comportar de forma muito semelhante a eles. 
...
```{r}
BD <- read.table(file="Data_frame_nn.txt", header=T)
```
Ler o banco de dados em formato .txt. Este é o mesmo BD que foi utilizado ao longo do livro. O arquivo deve estar no seu diretório de trabalho.

#### 2.3.5.3 Transformar as variáveis em fatores
```{r}
BD[,c("low","race","smoke","ptl","ht","ui","ftv")] <- lapply(BD[,c("low",
                                                                 "race",
                                                                 "smoke",
                                                                 "ptl",
                                                                 "ht",
...
#### 2.3.5.4 Adicionar subtítulos às variáveis
```{r}
var_label(BD)[1:11] <- c("Código de identificação",
                       "Baixo peso ao nascer",
                       "Idade materna (anos)",
                       "Peso no último ciclo mestrual (kg)",
                       "Raça da mãe",
                       "Tabagismo durante a gravidez",
                       "Partos prematuros prévios",
                       "Hipertensão materna",
                       "Irritabilidade uterina",
                       "Qtd de Pré-Natais no 1o sems",
                       "Peso ao nascer")
```

#### 2.3.5.5 Verificar a existência de NAs
```{r}
NAs <- is.na(BD);which(TRUE == NAs)
```

### 2.3.7 Intervalo de credibilidade
O intervalo de credibilidade, ou High Density Interval (HDI), é o equivalente Bayesiano do intervalo de confiança. Ele é derivado da distribuição posterior do parâmetro.
...
### 2.3.9 Correlação Linear

Para realizar uma correlação linear segundo uma inferência Bayesiana é preciso compilar nosso modelo em uma outra linguagem de programação que não o R, é preciso codifica-lo em Stan. Stan é uma linguagem de programação que tem como foco principal análises estatisticas, por isso é adequada para realizarmos as cadeias de Markov.

Primeiro é importante que entendamos como o Stan se estrutura. Um código em Stan tem quarto partes: data, parameters, model e generated quantities. A primeira, como o proprio nome sugere, se refere aos nossos dados, a segunda aos parametros, a terceira (model) são os priors e verossimilhança e na ultima dizemos o que queremos que o código gere, ou seja, o que estamos analisando. Em cada uma dessas partes precisamos declara variaveis ou atribuir suas distribuições, desse modo, por exemplo na parte data nós devemos declarar as variáveis de nossos que iremos usar, como N, nos parameters declaramos nosso desvio padrão e no model atribuimos uma distribuição a priori para esse desvio padrão.

Em Stan, nós atribuimos nossas variáveis segundo tipos númericos, como inteiro (int), real (real), vetor (vector), matriz, etc. Além disso ao final de cada linha deve haver um ponto e vírgula (;) e os comentários são feito com duas barras (//).
...
Vamos realizar uma correlação linear entre peso da mãe no ultimo ciclo menstrual e o peso do bebê ao nascer. Para isso utilizaremos uma código em Stan em funções do rstan para rodar as cadeias de Markov. Há duas maneiras de ncluir um código de Stan no R, através de um arquivo externo ou ao atribuir o código a uma variável string, utilizaremos aqui a segunda opção.
...
//matriz de covariancia
        cov_matrix[2] cov = [[ sigma[1] ^ 2, sigma[1] * sigma[2] * rho], [sigma[1] * sigma[2] * rho, sigma[2] ^ 2 ]];
...
vec_2 <- matrix(data = c(BD$lwt,BD$bwt), ncol = 2, nrow = 189)
lista <- list(N=nrow(BD), x = vec_2)
Corr_Lin <- stan(model_code = scode_cor, data = lista, 
                 iter = 2000, warmup = 500, chains = 4)
```

Perceba que uma outra parte surgiu no código em Stan, o "transformed parameters" isso acontece aqui, pois especificamente em relação a correlações lineares nós precisamos calcular a matriz de covariância, uma medida estatística que está "por trás" de qualquer análise de correlação, como Pearson e Spearman.