...
A verossimilhança deve ser baseada nos nossos dados, ou seja, a cada 20 mulheres que tomaram o anticoncepcional em média 4 ficam grávidas. Essa situação pode ser modelada por uma distribuição binominal com tamanho 20 (número de participantes) e uma probabilidade de sucesso baseada na variação do nosso parâmetro theta entre 0 e 1. Estamos interessados na probabilidade de encontrarmos 4 grávidas para cada valor que o parâmetro theta pode assumir.
...
Por fim, a probabilidade marginal, como na situação anterior, é igual à soma dos produtos priori * verossimilhança para cada possível valor de theta e equivale à probabilidade total de encontrarmos os dados X.
...
Como dito anteriormente, em casos com parâmetros contínuos a integral do denominador se torna muito complexa e dificulta a realização da equação. Esse é um dos principais motivos pelo qual a inferência bayesiana só se tornou popular com o advento dos computadores pessoais de alto processamento, ou seja a partir do final do século XX.
...
Além disso, é importante avaliar como o nosso modelo se comportou dentro dessa aleatoriedade e ele está adequada para tirarmos nossas conclusões para isso nós avaliamos a convergência do modelo. Vale ressaltar que medida, quase sempre necessária, é realizar um número suficiente de cadeias e de interações, geralmente acima de 3 e 1000, respectivamente, como um modo de aumentar a convergência e eficácia do modelo.
...
Um passo muito importante em uma análise de inferência bayesiana é a avaliar a convergência do modelo. Como dissemos acima o método MCMC se utiliza de uma inicialização aleatória para construir a distribuição a posterior, como podemos garantir que essa inicialização tenha convergido para a distribuição buscada?

Além disso, MCMC se utiliza do teorema do limite central, que em linhas gerais, afirma que quando amostras são suficientemente grandes elas terão uma distribuição normal, o que implica que as diferentes cadeias não podem estar correlacionadas, dessa forma também precisamos garantir isso para sustentar a eficácia de nosso modelo.

Para verificar a convergência e a autocorrelação do modelo utilizamos um indicador gráfico e dois numéricos, o traceplot, Rhat e neff, respectivamente. Os dois primeiros são utilizados para analisar convergência e neff para autocorrelação.

O traceplot é um gráfico traços que analisa as interações de cada cadeia de Monte Carlo, o gráfico se parece com um gráfico de frequência de rádio, ele demonstra por onde as interações da cadeia "andaram", por assim dizer. E um gráfico que aponte convergência vai ter traços bem condensados e mais ou menos na mesma faixa ao longo das interações, sobretudo após o warmup.

O Rhat é um outro indicador de convergência que é resultado da variação de todas as cadeias combinadas divido pela variação intracadeia, assim se há convergência as diversas cadeias irão seguir a mesma distribuição e o Rhat será 1. Ao contrário se não houver convergência a variação de todas as cadeias será maior que a de cada cadeia individualmente e o Rhat será maior q 1. Em regras gerais buscamos um valor de Rhat menor que 1.1.

Por fim, o neff, ou neff ratio, é um indicador para avaliar a autocorrelação das cadeias e representa, basicamente, a autoindependência das cadeias. Geralmente se está buscando um neff ratio maior que 0.01.
...
Já introduzimos o conceito de checagem preditiva posterior anteriormente. A ideia é simples: se um modelo é adequado para a descrição dos dados, devemos ser capazes de gerar novos dados através do modelo semelhantes aos dados observados, para isso precisaremos de alguns "truques". Primeiro criaremos uma matriz com nossos dados sem qualquer análise. Depois extrairemos de nosso modelo a variável gerada para a checagem preditiva do posterior a Z_rand. Por ultimo, plotaremos os dados da nossa matriz inicial e um elipse baseado no Z_rand, caso os pontos, ou boa parte deles, estejam dentro a área do elipse nosso modelo está adequado.
...
dataEllipse(x_rand, levels = 0.95, fill = T, plot.points = F)
```
