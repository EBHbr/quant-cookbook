...
Nessa sessão, precisaremos de alguns pacotes para além dos nativos do R. Além disso, para utilizar uma análise especifica é preciso baixar em seu computador um software chamado "JAGS". As bibliotecas serão carregada conforme uso.
```{r}
...
Vamos dar um passo adiante e pensar um contexto um pouco mais complexo que nos permita ver mais claramente a influência de cada um dos fatores do teorema de Bayes.

Antes precisamos entender o teorema substituindo A e B por um parâmetro (theta) e os dados (X), ficando, portanto, da seguinte forma: P(theta|X) = P((theta)*P(X|theta))/P(X).
...
Para isso temos que pensar da seguinte maneira, as probabilidades de se estar grávida tomando o anticoncepcional devem variar de 0 a 1, isto é, de 0% a 100%. Neste exemplo, trataremos probabilidade como uma variável discreta. Este é o nosso parâmetro theta, cuja distribuição posterior queremos calcular.

A verossimilhança deve ser baseada nos nossos dados, ou seja, a cada 20 mulheres que tomaram o anticoncepcional em média 4 ficam grávidas. Essa situação pode ser modelada por uma distribuição binomial com tamanho 20 (número de participantes) e uma probabilidade de sucesso baseada na variação do nosso parâmetro theta entre 0 e 1. Estamos interessados na probabilidade de encontrarmos 4 grávidas dentre 20 participantes para cada valor que o parâmetro theta pode assumir.

Como é uma situação hipotética, e podemos estabelecer nosso priori, comecemos com um prior não significativo que tenha a mesma probabilidade para todas as possibilidades, ou seja, que seja 1.

Por fim, a probabilidade marginal, como na situação anterior, é igual à soma dos produtos priori * verossimilhança para cada possível valor de theta e equivale à probabilidade total de encontrarmos os dados X, P(X).

Quando levamos isso para o código e transformamos em um data.frame fica da seguinte maneira:
```{r}
var_prob <- seq(0, 1, length.out = 101)
priori <- 1
vero <- dbinom(x = 4, prob = var_prob, size = 20)
prob_marginal <- vero * priori
posterior <- prob_marginal/sum(prob_marginal)

tabela1 <- data.frame(var_prob, priori, vero, prob_marginal, posterior)
```

Se você plotar essa tabela vai ver um apanhado de número poucos intuitivos e, para facilitar, vamos colocar tudo isso em gráficos. Mas antes, tente pensar: se você estivesse realizando esse teste e tivesse um prior pouco significativo, onde colocaria sua confiança? Muito provavelmente nos dados, certo? Já que eles são sua fonte de contato com o fenômeno.
```{r}
library(ggplot2)
ggplot(tabela1, aes(x=var_prob)) +
...
Veja que o teorema de Bayes realiza, matematicamente, justamente o que pensamos por intuição. Isto é, o posterior segue a tendência da verossimilhança, uma vez que o prior não tem significância. A curva do posterior está mais abaixo que a da verossimilhança pois esse é o que chamamos de posterior padronizado ou normalizado, uma vez que ele está sendo divido pela probabilidade marginal. Se tivéssemos uma linha nesse gráfico para o numerador da equação ela estaria exatamente sobre a linha da verossimilhança. Por fim, a linha do posterior aparece constante e em cima, justamente porque ele é 1 para todas as possibilidades.

Vamos prosseguir e mudar a situação um pouco. Digamos que, com os mesmos dados que obtivemos antes (ou seja, entre 20 participantes 4 ficaram gravidas), temos um priori significativo, confiável e que tem informações bem diferentes dos dados. Pensando que temos razões para confiar no priori e nos nossos dados, onde, por intuição, você colocaria sua distribuição posterior? Se você pensou entre os dados e o priori, você está entendendo a intuição do teorema.

Para realizar o cálculo dessa vez precisamos ter o priori em forma de distribuição. Digamos que nosso priori diz que em média 80% das mulheres engravidam, com um bom nível de confiança. Para isso teremos uma distribuição normal com média 0.8 e desvio-padrão 0.1 e o código ficará da seguinte maneira:
```{r}
priori <- dnorm(x = var_prob, mean = .8, sd = .1)
prob_marginal <- vero * priori
posterior <- prob_marginal/sum(prob_marginal)
tabela1 <- data.frame(var_prob, priori, vero, prob_marginal, posterior)
...
Por fim, vamos ao último exemplo dessa situação problema. O que aconteceria se tivéssemos dados poucos informativos, ou seja, se tivéssemos poucas observações, e uma priori mais condizente com observações anteriores? Por exemplo, uma amostra pequena de três participantes com duas grávidas e uma priori que indicasse uma probabilidade de 25% de se estar grávida. Vamos descobrir:
```{r}
priori <- dnorm(x = var_prob, mean = .25, sd = .1)
vero <- dbinom(x = 2, prob = var_prob, size = 3)
prob_marginal <- vero * priori
posterior <- prob_marginal/sum(prob_marginal)
tabela1 <- data.frame(var_prob, priori, vero, prob_marginal, posterior)
...
Mais uma vez, o teorema adequa a distribuição posterior conforme a intuição. Ou seja, a posterior fica mais próxima de nosso conhecimento prévio, que nesse caso é muito mais confiável que nossos dados. Essa, inclusive, é uma das vantagens de se usar a inferência Bayesiana. Com observações muito pequenas, pouco confiáveis, a utilização de um priori adequa a probabilidade posterior. Vale ressaltar que caso o priori e a verossimilhança sejam condizentes, o posterior vai se comportar de forma muito semelhante a eles.
...
Inicialmente, discutimos uma situação em que só haveria duas opções no nossos espaço probabilístico: Ou o indivíduo estava infectado ou não estava infectado. Depois, discutimos uma situação na qual tínhamos um espaço probabilístico com um parâmetro discreto que variava de 0 a 100.  Mas e quando o espaço probabilístico é contínuo e o parâmetro pode assumir infinitos valores?
...
Como dito anteriormente, em casos com parâmetros contínuos a integral do denominador se torna muito complexa e dificulta a realização da equação. Esse é um dos principais motivos pelo qual a inferência bayesiana só se tornou popular com o advento dos computadores pessoais de alto processamento, a partir do final do século XX.

A saída que se tem para a resolução do problema é a utilização dos algoritmos de MCMC. A ideia básica é reconstruir a distribuição posterior através de simulações de amostragem dela mesma. A lógica por trás desses cálculos é baseada no que chamamos de uma caminhada aleatória (random walk) que (re)avalia cada passo dado aleatoriamente de modo a identificar se ele é muito provável (portanto é aceito); se é menos provável, mas próximo o suficiente do ponto anterior (também é aceito); ou se ele tem uma probabilidade baixa com uma alta margem, quando é rejeitado e se sorteia um novo ponto aleatório. Esse é o principio por trás da estratégia de MCMC chamada Metropolis-Hastings.
...
Esse sistema parece, e é, complexo, mas não se incomode em focar em todos os detalhes, pois o computador fará todo o trabalho "sujo" por você. O que precisamos entender para compreender o suficiente dos nossos modelos é que essa é uma simulação de cadeia, que baseia-se no modelo construído e utiliza "passos aleatórios", os quais vão se tornando cada vez menos aleatórios com base nas (re)avaliações dos pontos, para reconstituir a distribuição posterior do parâmetro analisado. 

Desse modo, a quantidade de passos que vamos dar (iter) é importantes para a simulação da distribuição, bem como a quantidade de cadeias que teremos (chains), isto é, quantas vezes vamos repetir o número de passos. Por fim, vale ressaltar que devemos sempre nos atentar com os primeiros passos dados, pois devido a aleatoriedade eles geralmente são poucos adequados ao modelo e caminham em pontos menos prováveis, por isso, geralmente eliminamos os primeiros passos (warmup).

Vale ressaltar a importância de realizar um número suficiente de cadeias e de interações, geralmente acima de 3 e 1000, respectivamente, como um modo de aumentar a convergência e eficácia do modelo.

Essa é uma explicação simplificada e usando pouca, ou nenhuma matemática, do que é uma MCMC e uma HCM. Para entender com maior profundida esses dois conceitos, recomendamos o capítulo 9 do livro "Statistical Rethinking" (MCELREATH, 2015).

#### 2.3.5.2 Como analisar a convergência?
Um passo muito importante em uma análise de inferência Bayesiana é a avaliar a convergência do modelo. Como dissemos acima, o método MCMC se utiliza de uma inicialização aleatória para construir a distribuição posterior. Mas como podemos garantir que essa inicialização tenha convergido para a distribuição buscada?

Basicamente, para que a convergência ocorra, precisamos garantir que o Teorema do Limite Central se sustente. Para isso, precisamos de uma quantidade grande de amostras independentes na cadeia MCMC. No entanto, como no algoritmo de Metropolis-Hastings um ponto da amostra é dependente do ponto imediatamente anterior, pode ocorrer um fenômeno chamado de autocorrelação, já que as amostras não são todas independentes umas da outras. Precisamos, então, garantir que tenhamos uma quantidade suficiente de amostras independentes para que o TLC garanta a convergência da cadeia MCMC na direção da distribuição posterior.

Para verificar o grau de autocorrelação e a convergência e das cadeias MCMC, utilizamos um indicador gráfico e dois numéricos, o traceplot, Rhat e neff, respectivamente. Os dois primeiros são utilizados para analisar convergência e o neff para autocorrelação.

O traceplot é um gráfico traços que analisa as interações de cada cadeia de MCMC. O gráfico se parece com um gráfico de frequência de rádio, demonstrando por onde as interações da cadeia "andaram", por assim dizer. Um gráfico que aponte convergência vai ter traços bem condensados, com as cadeias convergindo mais ou menos na mesma faixa ao longo das interações, sobretudo após o warmup.

O Rhat é um outro indicador de convergência. Basicamente, ele é uma razão entre a variação intercadeia e a variação intracadeia. Caso haja convergência entre as cadeias, esperamos que a variação intercadeia seja bastante semelhante à intracadeia. Por isso, quanto maior a convergência, mais próximo de 1 será o Rhat. Ao contrário, se não houver convergência, a variação entre as cadeias será maior que a de cada cadeia individualmente e o Rhat será maior que 1. Como uma regra geral, buscamos um valor de Rhat menor que 1.1.

Por fim, o neff e neff ratio são indicadores para avaliar a autocorrelação das amostras. O neff é o tamanho amostral efetivo, que é basicamente a quantidade de amostras independentes das cadeias MCMC. O neff ratio é uma razão do tamanho amostral efetivo em relação ao total de iterações. Como regra geral, busca-se um neff ratio maior que 0.01. Em alguns modelos, os indicadores BulkESS e TailESS serão apresentados nos outputs. Eles representam o tamanho amostral efetivo para o centro e as caudas da distribuição, respectivamente.
...
Para realizar uma correlação linear segundo uma inferência Bayesiana, utilizamos a opção de compilar nosso modelo em uma outra linguagem de programação que não o R, chamada Stan. Stan é uma linguagem de programação que tem como foco principal análises estatísticas Bayesianas, por isso é adequada para realizarmos as cadeias MCMC. Stan utiliza o modelo hamiltoninano do MCMC.

Primeiro, é importante que entendamos como o Stan se estrutura. Um código em Stan tem quarto partes: data, parameters, model e generated quantities. A primeira, como o nome sugere, se refere aos nossos dados. A segunda (parameters) aos parâmetros. A terceira (model) são aos priors e verossimilhança. Por fim, na última (generated quantities), dizemos o que queremos que o código gere, ou seja, o que estamos analisando. Em cada uma dessas partes precisamos declarar variáveis ou atribuir suas distribuições.
...
Não se preocupe se forem muitas informações agora. O importante é você entender que é uma outra linguagem de programação e, por isso, tem suas especificidades. Ao longo da prática, algumas dúvidas vão desaparecer.

Vamos realizar uma correlação linear e para isso, lembre-se, precisamos de duas variáveis continuas. Utilizaremos um código em Stan e funções do rstan para rodar as cadeias de Markov. Há duas maneiras de incluir um código de Stan no R, através de um arquivo externo ou ao atribuir o código a uma variável string. Utilizaremos aqui a segunda opção.
```{r}
scode <- "data {
  //dados
...
Antes de tudo, você deve ter notado que estamos utilizando um modelo que tem como saída parâmetros da distribuição t de Student. Ao fazermos isso estamos seguindo uma recomendação de Baez-Ortega (2018), cujo texto que utilizamos como base para esse modelo pode ser encontrado neste link: https://baezortega.github.io/2018/05/28/robust-correlation/. Ele nos permite maior robustez para situações em que os dados apresentam grande quantidade de outliers. Contudo, você também deve ter percebido no código que é possível designar outras distribuições de modo bem simples com Stan. Logo, você pode escolher a que se adeque melhor aos seus dados.

Além disso, perceba que uma outra parte surgiu no código em Stan, o "transformed parameters". Nós utilizamos essa parte do código porque, especificamente em relação às correlações lineares, nós precisamos calcular a matriz de covariância, que está "por trás" de qualquer análise de correlação, como Pearson e Spearman, e que é um dos parâmetros das distribuições multivariadas que são a base destes modelos.

Para utilizar o Stan no R, é preciso seguir alguns passos que vão além da lógica do código em Stan. Primeiro é preciso construir uma matriz que contenha somente os dados a serem utilizados no modelo. Depois transformar essa matriz em uma lista, pois o rstan só aceita dados em forma de lista. Por fim é preciso criar uma variável com as cadeias MCMC e para realizar as cadeias é preciso utilizar a função stan, presente no pacote rstan. Nos argumentos dessa função você deve indicar o código em Stan, os dados em forma de lista, o número de "passos" a serem dados, quantos passos iniciai serão descartados e quantas cadeias haverão, respectivamente.

#### 2.3.8.3 Verificação dos convergência
Como dito anteriormente, para realizar uma análise em inferência bayesiana, precisamos verificar algumas coisas antes, a convergência e a independência entre as cadeias.
...
Outro modo de avaliar a convergência é através do Rhat. Você pode observar esse indicador ao printar o modelo, ele estará na ultima coluna, ou pode separa-lo em uma nova variável:
```{r}
print(Corr_Lin)
rhats <- rhat(Corr_Lin)
print(rhats)
...
Por fim, temos a verificação de independência das amostras, o neff ratio, ele também estará presente ao printar o modelo, dessa vez na penultima coluna, porém você continua podendo separar em uma outra variável
```{r}
print(Corr_Lin)
...
#### 2.3.8.5 Checagem preditiva posterior
Já introduzimos o conceito de checagem preditiva posterior anteriormente. A ideia é simples: se um modelo é adequado para a descrição dos dados, devemos ser capazes de gerar novos dados através do modelo semelhantes aos dados observados, para isso precisaremos de alguns "truques". Primeiro criaremos uma matriz com nossos dados sem qualquer análise. Depois extrairemos de nosso modelo a variável gerada para a checagem preditiva do posterior a Z_rand. Por ultimo, plotaremos os dados da nossa matriz inicial e um elipse baseado no Z_rand. Caso cerca de 95% dos pontos dos dados originais estejam dentro a área do elipse, consideramos que nosso modelo está adequado.
```{r}
Matriz_Dados <- matrix(data = c(X$a,X$b), ncol = 2, nrow = length(X))
Z_rand <- extract(Corr_Lin, c("Z_rand"))[[1]]
plot(Matriz_Dados, xlim = c(10, 120), ylim = c(0, 5200), pch = 16)
dataEllipse(Z_rand, levels = 0.95, fill = T, plot.points = F)
```
...
#### 2.3.8.3 Verificação dos convergência
Pode ser feita com os mesmos códigos apresentados na parte de análise de convergência da correlação.

#### 2.3.9.4 Análise gráfica dos parâmetros
...
Novamente, a ideia é simples: se um modelo é adequado para a descrição dos dados, devemos ser capazes de gerar novos dados através do modelo semelhantes aos dados observados.
