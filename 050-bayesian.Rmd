...
```{r}
varprob <- seq(0, 1, length.out = 100)
priori <- 1
...
```{r}
priori <- dnorm(x = varprob, mean = .8, sd = .1)
prob_marginal <- vero * priori
posterior <- prob_marginal/sum(prob_marginal)
tabela1 <- data.frame(varprob, priori, lik, prob_marginal, posterior)
...
```{r}
priori <- dnorm(x = varprob, mean = .25, sd = .1)
vero <- dbinom(x = 2, prob = varprob, size = 3)
