...
Inicialmente, discutimos uma situação em que só haveria duas opções no nossos espaço probabilístico: Ou o indivíduo estava infectado ou não estava infectado. Depois, discutimos uma situação na qual tínham um espaço probabilístico com um parâmetro discreto que variava de 1-100.  Mas e quando o espaço probabilístico é contínuo e o parâmetro pode assumir infinitos valores?

Nesses casos, a nossa probabilidade marginal é calculada através de uma integral.....

### 2.3.6 Baixando e ajustando nosso BD
### 2.3.5 Baixando e ajustando nosso BD
...
Bayes_X_NI
```
X é a variável do nosso BD para a qual queremos calcular parâmetros. O output da função nos dará a estimativa pontual do parâmetro da população (mean), o desvio padrão (sd), a mediana (median) e os limites inferior (HDIlo) e superior (HDIup) do intervalo de credibilidade da distribuição posterior para a média (mu) e o desvio padrão (sigma) da população.
...
Agora, vamos fazer um processo similar ao teste t, porém utilizando a estimativa Bayesiana das distribuições posteriores das médidas de cada grupo, assim como da diferença entre elas.
